...
type SiteConfig struct {
	...
	BackupEnabled bool   `gorm:"column:backup_enabled;not null;default:false;comment:自动灾备开关"`
	BackupTime    string `gorm:"size:5;column:backup_time;not null;default:02:00;comment:每日灾备时间"`
	BackupPrefix  string `gorm:"size:256;column:backup_prefix;not null;default:'';comment:灾备对象前缀"`
	// 题库 AI 分析（OpenAI 兼容）
	...
	SpiderConcurrency         int    `gorm:"column:spider_concurrency;not null;default:4;comment:OJ 爬虫消费并发"`
	ProblemFetchConcurrency   int    `gorm:"column:problem_fetch_concurrency;not null;default:4;comment:题面抓取消费并发"`
	ProblemAnalyzeConcurrency int    `gorm:"column:problem_analyze_concurrency;not null;default:4;comment:题库分析消费并发"`
	// 又拍云图床（博客/题解）；密码在受控数据库中明文存储
	...
	UpyunPassword string `gorm:"size:512;column:upyun_password;comment:又拍云操作员密码"`
	...
	PayFmSecret     string `gorm:"type:text;column:payfm_secret;default:'';comment:支付FM接入密钥"`
